...
class Main extends luxe.Game {
    ...
    override function ready() {
        ...
        // MAKE a random one near the center Gold?
        // var rndY = Math.round(this.gridHeight/2 + (Math.random()-0.5)*10);
        // var rndX = Math.round(this.gridWidth/2 + (Math.random()-0.5)*10);
        // model[rndY][rndX].bl = new Color().rgb(0xDAA520).toColorHSL();
    ...
    function generateColourFor(y,x,isBottomLeft){
        ...
        //select a patter
        pattern = pattern_model.cubeOneNoPaddingWrap;
        ...
        var c:Color =  new Color().rgb(colours[cellPatternVal-1]); 
    ...
    function before(_) {
        ...
        Luxe.renderer.clear(new Color().rgb(0x000000));
    ...
    function startTweens() {
        for(i in 0...gridHeight){
            for(j in 0...gridWidth){
                var opt:ColorOptions = {h:modelTo[i][j].bl.h, s:modelTo[i][j].bl.s, l:modelTo[i][j].bl.l};
                geometries[i][j].bl.color.tween(animationTime,opt);


                var optTwo:ColorOptions = {h:modelTo[i][j].tr.h, s:modelTo[i][j].tr.s, l:modelTo[i][j].tr.l};
